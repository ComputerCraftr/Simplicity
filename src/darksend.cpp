    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                adds up to a cost of 0.002LOC per transaction on average.
            */
            if(r <= 20)
            {
...
bool CDarksendPool::DoAutomaticDenominating(bool fDryRun, bool ready)
{
    ...
    int64_t nBalanceNeedsAnonymized = nAnonymizeSimplicityAmount*COIN - pwalletMain->GetAnonymizedBalance();
    ...
    if(!sessionFoundMasternode){
        ...
        double fSimplicitySubmitted = (sessionTotalValue / COIN);
        LogPrintf("Submitting Darksend for %f TX COIN - sessionTotalValue %d\n", fSimplicitySubmitted, sessionTotalValue);
...
void CDarksendPool::GetDenominationsToString(int nDenom, std::string& strDenom){
    ...
    //
    // bit 0 - 100LOC+1 ( bit on if present )
    // bit 1 - 10LOC+1
    // bit 2 - 1LOC+1
    // bit 3 - .1LOC+1
    // bit 3 - non-denom
...
void ThreadCheckDarkSendPool()
{
    ...
    RenameThread("simplicity-darksend");
        ...
        if(c % 60*(nLiquidityProvider+1) == 0){
            if(nLiquidityProvider!=0){
                ...
                if(nRand == 50+nLiquidityProvider && pwalletMain->GetAverageAnonymizedRounds() > 8){
                    ...
                    nAnonymizeSimplicityAmount = (rand() % nLeftToAnon)+3;
                } else {

...
const string strMessageMagic = "Simplicity Signed Message:\n";
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags, bool fValidateSig)
{
    ...
    if (!IsCoinBase())
    {
        ...
        if (!IsCoinStake())
        {
            ...
            unsigned int txSize = ::GetSerializeSize(*this, SER_NETWORK, PROTOCOL_VERSION);
            int64_t nRequiredFee = GetMinFee(*this, txSize, GMF_RELAY);
...
#ifdef ENABLE_WALLET
// Simplicity: attempt to generate suitable proof-of-stake

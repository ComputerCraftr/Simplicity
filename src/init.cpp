...
void Shutdown()
{
    ...
    RenameThread("simplicity-shutoff");
...
std::string HelpMessage(HelpMessageMode hmm)
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: simplicity.conf)") + "\n";
    strUsage += "  -datadir=<dir>         " + _("Specify data directory") + "\n";
    strUsage += "  -testnet               " + _("Use the test network") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: simplicityd.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 11957 or testnet: 21957)") + "\n";
    strUsage += "  -maxconnections=<n>    " + _("Maintain at most <n> connections to peers (default: 8192)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 11958 or testnet: 21958)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -masternodeminprotocol=<n> " + _("Ignore masternodes less than version (example: 70020; default : 0)") + "\n";
    ...
    strUsage += "  -anonymizesimplicityamount=<n> " + _("Keep N Simplicity anonymized (default: 0)") + "\n";
...
{
    RenameThread("simplicity-loadblk");
...
#define PROCESS_DEP_ENABLE 0x00000001
...
#endif
    // Make sure only a single Simplicity process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Simplicity is probably already running."), strDataDir));
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Simplicity") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Simplicity to complete") << "\n";
    ...
    nAnonymizeSimplicityAmount = GetArg("-anonymizesimplicityamount", 0);
    if(nAnonymizeSimplicityAmount > 999999) nAnonymizeSimplicityAmount = 999999;
    if(nAnonymizeSimplicityAmount < 2) nAnonymizeSimplicityAmount = 2;
    ...
    LogPrintf("Anonymize Simplicity Amount %d\n", nAnonymizeSimplicityAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1SPL+1000 == (.1SPL+100)*10
       10SPL+10000 == (1SPL+1000)*10
    */
